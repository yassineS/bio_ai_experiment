...
func TestFQZComp_ComplianceVectors(t *testing.T) {
	...
	if ran == 0 {
		t.Skipf("htscodecs submodule not initialised — fqzcomp vectors unavailable; run `git submodule update --init reference_code/htscodecs`")
	}
}
...
func TestFQZComp_RoundTrip(t *testing.T) {
	...
	if ran == 0 {
		t.Skipf("htscodecs submodule not initialised — fqzcomp corpus unavailable; run `git submodule update --init reference_code/htscodecs`")
	}
}
