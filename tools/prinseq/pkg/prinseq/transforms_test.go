...
// upstreamPrinseqScriptPath returns the path to the vendored prinseq-lite.pl, or ""
// if the submodule is not checked out.
func upstreamPrinseqScriptPath(t *testing.T) string {
	t.Helper()
...
func perlAvailable(t *testing.T) (perl, script string, ok bool) {
	...
	script = upstreamPrinseqScriptPath(t)
	if script == "" {
		return "", "", false
	}
...
func TestParityTransforms(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			got := runGoFilterXform(t, tc.fixture, tc.isFastq, tc.flags)
			if got != want {
...
// runGoFilterXform runs the Go Filter with options derived from upstream-style
...
func runGoFilterXform(t *testing.T, fixture string, isFastq bool, flags []string) string {
	t.Helper()
	opts := FilterOptions{}
	lineWidthSet := false
	for i := 0; i < len(flags); i++ {
		...
		case "-min_len":
			...
			opts.MinLen = n
		default:
			t.Fatalf("runGoFilterXform: unhandled flag %q", flags[i])
		}
	}
