...
**Status: RESOLVED.** The in-tree pure-Go libdeflate port landed
(Slices 0–4) in `pkg/htsgo/libdeflate`. `DeflateRaw(src, level)`
produces raw DEFLATE byte-identical to genuine libdeflate — proven by
the oracle tests in `pkg/htsgo/libdeflate/` that compare against real
libdeflate `.gz` outputs generated by the C trace harness in
`reference_code/libdeflate_trace/` (which links the actual libdeflate
submodule). `pkg/htsgo/bgzf` now routes block compression through
`libdeflate.DeflateRaw`, so every BGZF block this project emits is
byte-identical to htslib's libdeflate output. The scope assessment
below is retained for historical context.

### Motivation (historical)

Several test cases documented themselves as "honest non-parity by
policy" because upstream samtools/bcftools link against `libdeflate`,
while htsgo's BGZF writer used Go's stdlib `compress/flate`.
Semantically the output was a valid DEFLATE stream; byte-for-byte it
diverged because of different match-finder heuristics and Huffman tree
construction.

- `tools/bcftools/pkg/bcftools/parity_test.go` `.tbi` byte equality —
  **RESOLVED**. The former `TestParityIndex_BinaryMatch` skip is now
  `TestParityIndex_TBIByteEqual`, which asserts byte-stability against a
  committed golden (`testdata/parity/index_tbi/expected.tbi`, produced by
  our libdeflate-backed pipeline) plus structural validity. Upstream
  byte-equivalence is carried transitively by the `pkg/htsgo/libdeflate`
  oracle tests.
- `tools/mosdepth/pkg/mosdepth/parity_test.go` `.csi` byte equality —
  **RESOLVED**. The former `TestParity_IndexFiles_Skipped` skip is now
  `TestParity_CSIByteEqual` (same regression-golden +
  transitive-equivalence pattern;
  `testdata/parity/index_csi/expected.csi`).
- `tools/samtools/pkg/samtools/calmd_test.go::TestParity_Calmd_UpstreamCorpus`
  and `import_test.go` — these assert logical parity (BGZF magic +
  per-record decode / round-trip) and were never gated on a
  libdeflate-blocked `t.Skip`, so they were left as-is.

Note: upstream htslib/samtools cannot be built or fetched in this
environment (htslib submodule clone needs GitHub auth; autoconf absent),
so the `.tbi`/`.csi` goldens are produced by our own libdeflate-backed
pipeline rather than captured from a genuine upstream binary. Upstream
byte-equivalence is therefore established transitively through the
`pkg/htsgo/libdeflate` oracle corpus, not by a direct upstream capture.
