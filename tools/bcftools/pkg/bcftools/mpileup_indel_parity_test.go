...
func upstreamBcftoolsMpileupIndel(t *testing.T) (bin string, srcMissing bool) {
	...
	s.once.Do(func() {
		...
		if out, runErr := runMake(hts); runErr != nil {
			s.err = wrapBuildErr("htslib", out, runErr)
			return
		}
		if out, runErr := runMake(root); runErr != nil {
			s.err = wrapBuildErr("bcftools", out, runErr)
			return
		}
		if fi, statErr := os.Stat(bin); statErr != nil || fi.Mode()&0o111 == 0 {
			s.err = statErr
...
// wrapBuildErr decorates a build failure with a tail of the build log.
func wrapBuildErr(what string, out []byte, err error) error {
	tail := string(out)
	if len(tail) > 2000 {
		tail = tail[len(tail)-2000:]
	}
	return &buildError{what: what, tail: tail, err: err}
}
