...
| **fastp** | single cmd; sliding-window, auto-adapter, HTML+JSON, dup-eval, UMI, PE base `--correction`, overrepresentation (`-p/-P`), `--split*`, merge writer (`-m`), `--adapter_fasta`, `--poly_x_min_len`, `--disable_adapter_trimming`, `--merge` (`merged_and_filtered` JSON block); poly-G/poly-X + sliding-window byte-exact; SE adapter auto-detect similarity-bounded (observed byte-identical) via `trimBySequence` fix; 17/17 + 9 tail parity | multi-thread `--split` file-boundary distribution (perf) | **~99%** | small |
