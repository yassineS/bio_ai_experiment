...
3. **Scattered option-tail polish — effectively closed.** bcftools
   ...
   `tview` are upstream-dead/non-pipeline). (`som` is now shipped with
   the upstream write bug fixed.)
