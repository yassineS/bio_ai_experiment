...
| ------------- | -----------:| ------:| -------:| ----- |
| bedclosest    |          14 |     14 |       0 | The "close-the-deferreds" wave (this PR) closed all six remaining skips: `-N` (force different names), `-s`/`-S` strand filters (skip B's that fail the strand match), and the multi-DB family `-b A B C` with optional `-names` / `-filenames` labels, exposed via the new `ClosestMulti(readerA, dbReaders, writer, opts, dbLabels)` entry point. The B-side null placeholder is now sized to the database's observed BED width (Bed{3,4,5,6,12}::printNull). |
| bedcomplement |          11 |     10 |       1 | Skip remaining: the upstream out-of-range warning message format only. The "close-the-deferreds" wave (this PR) added `ComplementWithOptions(..., Options{LimitToInput: true})` for `-L` (issue #503), which restricts the emit order to chromosomes that had at least one record in the input. |
| bedflank      |          11 |     11 |       0 | All upstream cases (`flank.t1..t11`) covered. |
| bedgenomecov  |           9 |      3 |       6 | Skips: BAM/SAM/CRAM input, `-pc`, `-fs`, `-split`. |
| bedintersect  |          13 |     13 |       0 | The "close-the-deferreds" wave (this PR) closed all five remaining skips: `-wa -wb` (side-by-side A and B), `-wo` (write overlap length), `-wao` (write all, with the Bed3/4/5/6/12 null placeholder for misses), and the strand-aware `-s` variants of those, exposed via `IntersectOptions.WriteOverlap` / `WriteAllOverlap` plus combined `WriteA + WriteB`. The output stream for the combined modes is routed through a raw `bufio.Writer` so the A and B columns can sit jammed side-by-side without going through the BED-shape-aware `bed.Writer`. |
| bedjaccard    |          14 |     12 |       2 | Skips: BAM input, large fixture t16 (long.bed/short.bed). The "close-the-deferreds" wave (this PR) closed t08 / t12 / t13 by adding two stackable wrapper readers (`splitReader`, `strandFilterReader`) in front of the merging sweep, exposed via `Options.Split` and `Options.StrandFilter`. |
| bedmerge      |          16 |     14 |       2 | Skips remaining: VCF and GFF input (both out-of-scope for a BED tool). The "close-the-deferreds" wave (this PR) closed t10 (`-delim`) by threading `MergeOptions.Delim` through `applyOpDelim` / new `ApplyOpDelim`, and closed t16/t17 (`-S +` / `-S -`) by adding `MergeOptions.StrandFilter` (filters records up-front, both in the basic and column-ops merge paths). |
...
| bedsubtract   |          13 |     13 |       0 | The "close-the-deferreds" wave (this PR) closed all three `-N` skips (t9/t10/t21) via the new `Options.UnionDrop` toggle, which switches `MinFraction` from per-B-fraction to "union coverage > fraction ⇒ drop A entirely" — matching upstream `subtractFile.cpp`'s `getRemoveSum()` branch exactly (strict `>`, not `>=`). |
...
| stats      |           8 |      8 |       0 | SN-section byte parity on fixtures 1, 2, 5, 7, 8, 10 from `reference_code/samtools/test/stat/`. CHK/FFQ/LFQ/GCF/GCL/GCC/GCT/FBC/FTC/LBC/LTC/IC/ID/IS/COV/GCD are byte-faithful on the same fixtures; MPC is byte-faithful on fixtures 1/2/5/7/8 (`-r test.fa`); RFS is byte-faithful on fixtures 16/17/19 (`--ref-stats`). The per-barcode `<tag>C`/`<tag>Q` tables are byte-faithful on `13_barcodes_ok.sam` and `13_barcodes_ok_ox_bz.sam` (test.pl:3325-3326); the malformed-barcode `expect_fail` inputs (test.pl:3327-3329) are exercised for warn-and-skip behaviour. `--sparse` thins all-zero IS rows only. Full 1:1 section parity. |
