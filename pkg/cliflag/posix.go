...
// Parse normalizes args with Normalize and then calls fs.Parse on the result,
// giving any tool wired through it POSIX getopt-compatible short-flag bundling
// ("-bS" == "-b -S") and value concatenation ("-q20" == "-q 20") on top of
// Go's native long-flag handling. It is a drop-in replacement for
// fs.Parse(args): on the error path the caller should print usage and exit
// with code 2, exactly as it would for a plain fs.Parse failure.
