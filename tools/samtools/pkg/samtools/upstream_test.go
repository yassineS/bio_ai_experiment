...
// real upstream `samtools` binary alongside the Go port on the same input
...
// live. Accordingly the helper builds samtools from the vendored
// reference_code/ submodule on demand (recursive submodule init so
// htslib's nested htscodecs submodule is present, htslib built with its own
// ./configure, samtools built with plain `make` so it does not clobber
// htslib's config.mk) and t.Fatalf — never t.Skip — if the build fails.

var (
	samtoolsBinOnce sync.Once
	samtoolsBinPath string
	samtoolsBinErr  error
)
...
// upstreamSamtools returns the absolute path to a freshly built upstream
// `samtools` binary, building it (and htslib) from the vendored submodules
...
func buildSamtools() (string, error) {
	...
	samtools := filepath.Join(root, "reference_code", "samtools")
	bin := filepath.Join(samtools, "samtools")
	...
	// samtools: plain make (NOT ./configure, which would clobber htslib's
	// config.mk). The Makefile picks up the sibling ../htslib by default.
	if !fileExists(filepath.Join(samtools, "bamtk.c")) {
		if out, err := run(root, "git", "submodule", "update", "--init", "--recursive", "reference_code/samtools"); err != nil {
			return "", wrapBuild("git submodule samtools", out, err)
		}
	}
	if out, err := run(samtools, "make", "-j4"); err != nil {
		return "", wrapBuild("make samtools", out, err)
	}
	if !fileExists(bin) {
		return "", errf("samtools binary not produced at %s", bin)
	}
	return bin, nil
}
