...
// parseHapChromPosRefAlt splits a CHROM:POS_REF_ALT[_END] token into its
// components. END, when present, is returned in end (0 means absent).
func parseHapChromPosRefAlt(tok string) (chrom string, pos int, ref, alt string, end int, err error) {
...
func parseLegendLine(line, chrom string) (*vcf.Variant, bool, error) {
	...
	c, pos, ref, alt, end, err := parseHapChromPosRefAlt(fields[0])
	if err != nil {
...
func parseHapSampleLine(line, chrom string, samples []string) (*vcf.Variant, error) {
	...
	c, pos, ref, alt, end, err := parseHapChromPosRefAlt(fields[1])
	if err != nil {
		return nil, err
	}
	if c != chrom {
