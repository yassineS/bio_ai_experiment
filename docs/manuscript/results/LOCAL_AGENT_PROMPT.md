...
SOURCE OF TRUTH: read docs/manuscript/results/HEAVY_TIER_RUNBOOK.md in full and
...
out of the git-ignored pipeline/.fixtures/ into tracked docs/manuscript/results/;
...
1. REAL-DATA parity + performance (multi-contig). Run:
     ...
     go run ./pipeline/cmd/realparity -ref $REF -bam $BAM -vcf $VCF -reps 5 \
       -out "$PWD/docs/manuscript/results/realdata/HG002_GRCh38" -v
...
2. LARGE-TIER parity + round-trip interop + performance (full flag matrix, all
   ...
   docs/manuscript/results/large_tier/performance.md: per-cell median ours/upstream
   ...
   2b. HARDWARE SPEC (required). Write docs/manuscript/results/large_tier/
...
3. K-RUN REPRODUCIBILITY — ONLY if you have LLM-API budget for it (it re-runs the
   ...
   interventions, tokens/$; write docs/manuscript/results/reproducibility/krun.md.
...
tracked docs/manuscript/results/ tree; run
`npx --yes markdownlint-cli2@0.13.0 "docs/manuscript/results/**/*.md"` (0 errors);
