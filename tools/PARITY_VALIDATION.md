...
| stats      |           6 |      6 |       0 | SN-section byte parity on fixtures 1, 2, 5, 7, 8, 10 from `reference_code/samtools/test/stat/`. CHK/FFQ/LFQ/GCF/GCL/GCC/GCT/IC/ID/COV/GCD are byte-faithful on the same fixtures; MPC is byte-faithful on fixtures 1/2/5/7/8 (`-r test.fa`); RFS is byte-faithful on fixtures 16/17/19 (`--ref-stats`). Only the FBC/FTC/LBC/LTC barcode tables remain deferred. |
