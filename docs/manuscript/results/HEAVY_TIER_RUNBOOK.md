...
`pipeline/.fixtures/` tree into a tracked `docs/manuscript/results/<experiment>/`
...
3. Re-run H1 with `-reps>=10` for stable quantiles and write
   `docs/manuscript/results/large_tier/performance.md`: per-cell median ratio +
...
4. Report `J/K` reached parity per unit and the intervention distribution in
   `docs/manuscript/results/reproducibility/krun.md`. This is the honest
...
1. Copy every report out of the git-ignored `pipeline/.fixtures/` into the
   ...
   ```bash
   mkdir -p docs/manuscript/results/large_tier
   cp pipeline/.fixtures/large/full-validation/*.{md,json} docs/manuscript/results/large_tier/
   ```
...
- **Second independent labeler** for the bug-corpus κ (claim C7): re-label
  `docs/manuscript/bug_corpus.md` independently and compute inter-rater κ.
