...
func runSingleEnd() {
	...
	cliflag.IntVar(fs, &windowSize, "w", "window-size", 0, "Window size for quality assessment (default: dynamic, int(0.1*read_length) per read)")
	...
	fs.Usage = func() {
		...
		fmt.Fprintf(os.Stderr, "  -w, --window-size INT       Window size for quality assessment (default: dynamic, int(0.1*read_length) per read)\n")
