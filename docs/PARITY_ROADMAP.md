...
Closed in the live-oracle wave (this PR):

- **`--freq` / `--counts` / `--het` / `--missing-site` byte parity vs a
  freshly-built genuine VCFtools 0.1.18 binary.** A new live oracle
  (`tools/vcftools/pkg/vcftools/testdata/live/`, `in.vcf` +
  `expected.*` generated by `reference_code/vcftools/src/cpp/vcftools`)
  is pinned byte-for-byte by `TestVcftoolsLiveOracle`. Three divergences
  were fixed:
  - **`--freq`**: frequency columns used a fixed `%.6f`
    (`A:0.500000`) instead of upstream's default ostream `<<`
    minimal-significant-digit formatting (`A:0.5`,
    `variant_file_output.cpp:135`). Now routed through
    `formatCppDouble` (`'g'`, 6 sig-digits). `--counts` was already
    integer-formatted and unaffected.
  - **`--het`** (`statistics.go` `addHetStat`/`outputHet`): E(HOM) is the
    per-site expected-homozygosity sum
    `Σ (1 − 2·p·q·N/(N−1))` over included biallelic SNP sites, and
    `F = (O − E)/(N − E)` (`variant_file_output.cpp:240-280`); E(HOM)
    prints at fixed precision 1, F at precision 5. (This had already
    been corrected in code; the live oracle now guards it.)
  - **`--missing-site`** (`statistics.go` `addSiteMissingStat`/
    `outputMissingSite`): `N_DATA` is the allele count `site_N_tot`
    (2 per diploid sample, decremented for haploid / phased-missing
    genotypes — `variant_file_output.cpp:893-924`), not the sample
    count; `N_MISS` counts missing alleles; `F_MISS = N_MISS/N_DATA`
    formats via the default ostream `<<` (`0`, not `0.000000`). The
    stale `freq.expected.frq`, `het.expected.het`,
    `missing_site.expected.lmiss`, and `derived.expected.frq` parity
    fixtures (which encoded the old `%.6f` / sample-count format) were
    regenerated; the het and missing-site fixtures are now
    byte-identical to the genuine binary on `sample.vcf`.
  - Pre-existing `--counts` / `--site-pi` / `--TsTv-summary` parity is
    preserved (the live oracle includes them as regression guards).
  - The freq/missing-site *row set* on multi-allelic / monomorphic
    sites still reflects the port's `--min-alleles`/`--max-alleles`
    handling (tracked separately under the column-set gaps); only the
    per-row formatting and the `--het`/`--missing-site` arithmetic were
    in scope here.
...
- **PASS (byte-equal modulo provenance):** `view` (bare, `-v snps`, `-v
  ...
  `annotate` (`-x INFO/AF`, `-x FORMAT/GQ`, `-I +%CHROM_%POS`,
  `-i AC>1`, `-e QUAL<10`, `-H ##test=v` (repeatable), `-h FILE`,
  `-s S1`), `head` (bare,
