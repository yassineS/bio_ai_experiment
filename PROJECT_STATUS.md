...
3. **Scattered option-tail polish — effectively closed.** bcftools
   ...
   long-flag gaps. The `convert` PLINK exporters are now implemented to
   the PLINK1 spec. What is left are documented **non-goals** (CRAM v4.0
   awaits a finalised spec; gtcheck `-c/--cluster`, `som`/`tview` are
   upstream-dead/non-pipeline).
