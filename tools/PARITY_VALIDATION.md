...
| markdup    |           4 |      4 |       0 | Byte-for-byte parity on 5_markdup and 6_remove_dups; flag+qname parity on 18_primary_duplicate_count (we don't emit `dt:Z:` tag); sequence-mode dup-count parity on a duplicate of fixture 5. Optical-duplicate detection (`-d/--max-dist`), the Picard-style stats report (`-s`), and supplementary marking (`-S`) are now implemented. |
