...
| G6 | **`bcftools isec` multi-sample FORMAT over-decode** | C3 | low | memory **fixed** (streaming k-way merge + byte-bounded batch: RSS 39×→~6.5×, multi-GB→~80 MB), and the apparent ~15× wall was a **slow-bind-mount + under-buffered-write artifact**, now fixed (256 KiB buffers on the VCF + sites.txt writers). On a fast disk isec is a steady ~2.1–2.5×; the residual is partly decoding multi-sample FORMAT columns the set op never uses (a lazy / raw-passthrough record path would trim it). |
