...
func TestArith_U32ComplianceVectors(t *testing.T) {
	...
	if err != nil {
		t.Skipf("htscodecs submodule not initialised — u32 corpus unavailable; run `git submodule update --init reference_code/htscodecs`")
	}
	ran := 0
	...
	if ran == 0 {
		t.Skipf("htscodecs submodule not initialised — u32 vectors unavailable; run `git submodule update --init reference_code/htscodecs`")
	}
}
