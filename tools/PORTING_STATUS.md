...
| `bedclosest` | `bedtools closest` | 92.8% | Sweep on sorted input; `-D ref/a/b`, `-N` (different-names), `--require-overlap`, `-t all/first/last`, `-s`/`-S` strand filters, multi-DB `-b ...`/`-names`/`-filenames`/`-mdb each\|all` |
