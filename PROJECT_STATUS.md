...
| **bedtools** | 37 bed* tools; no missing subcommands; 141+ parity tests; BAM **and CRAM** input (`bedintersect`/`bedmulticov`), VCF/GFF input (`bedintersect`/`bedmultiinter`), `bedclosest` direction flags, `intersect -c`; **`bedcoverage --split` over a blocked query (`-a`)** — a BED12 record or a spliced (`N`-CIGAR) BAM alignment — across every mode (default/`--counts`/`--depth`/`--hist`/`--mean`, `-s`/`-S`), byte-validated vs upstream 2.31.1 (intron bases excluded from overlap but kept in the full-span length/depth vector; B straddling an intron counted per overlapped block; `-f`/`-F`/`-r`/`-e` correctly ignored under `--split` as upstream does; BED12 `blockSizes`/`blockStarts` echoed verbatim; `-e` added) | no known concrete gaps — parity suites pass byte-for-byte across all 37 tools; zero divergence `t.Skip`s; bedsample is byte-exact (`std::mt19937_64` port). The remaining margin is a conservative polish hedge, not a list of missing flags. | **~99%** | none known |
| **vcftools** | single cmd; **146/146 upstream long flags (100%)**, incl. BCF I/O, PCA, LD, RoH, relatedness, `--freq2`/`--counts2` (schema complete) | no known concrete gaps — all 146 flags implemented and byte/precision-validated; no flag returns "not implemented" at runtime; zero divergence `t.Skip`s. `--max-indv` ports upstream's glibc `rand()` + `std::random_shuffle`, byte-exact for `--max-indv-seed` (upstream itself is time-seeded). Remaining margin is a conservative hedge for untested output-column edge cases, not missing features. (The old `tools/vcftools/ROADMAP.md` "~48%/not-implemented" text was stale and has been corrected.) | **~99%** | none known |
