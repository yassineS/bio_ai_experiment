...
```sh
nextflow run nextflow/main.nf --tiers chr20,exome,wgs ...
```
...
```sh
nextflow run nextflow/main.nf -profile test,standard
```
...
3. **Launch from the CLI** with the Tower CLI:
   ...
   tw launch https://github.com/yassineS/bio_ai_experiment \
       ...
       --profile      awsbatch \
       --params-file  nextflow/conf/params.example.yaml \
       --config       nextflow/nextflow.config
   ```

   or **from the Platform UI**: *Launchpad → Add pipeline →* point at this repo,
   set `nextflow/main.nf` as the main script, choose the `awsbatch` profile, and
...
```yaml
# nextflow/conf/params.example.yaml
tiers:        [chr20, exome]
reps:         3
