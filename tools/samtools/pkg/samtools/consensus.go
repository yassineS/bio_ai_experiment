...
const (
	...
	// ConsensusFASTQ emits one FASTQ record per contig; quality bytes carry
	// the per-position call confidence (Phred+33, capped at 93).
	ConsensusFASTQ
	// ConsensusPileup emits the samtools consensus pileup row format:
	// "chrom\tpos\tnth\tdepth\tcall\tcq\tseq\tqual\n" where nth is always
	// 0 in v1 (insertion-column rows not yet supported).
	ConsensusPileup
)
...
const (
	...
	// warning and falls back to ConsensusModeSimple.
...
// default. The upstream CLI default is 0 for consensus (distinct from
// mpileup's 13); we keep 0 to match.
const DefaultConsensusMinBaseQ = 0

// ConsensusOptions configures samtools consensus. Zero-valued defaults
// approximate the upstream `samtools consensus -m simple` runtime.
type ConsensusOptions struct {
	...
	// Mode picks the per-position algorithm (CLI -m/--mode).
	Mode ConsensusMode
	...
	Regions []string
	// BEDPath is a BED file restricting emission (CLI -l).
	BEDPath string
	// MinDepth filters out positions with strictly less than this many
	// usable reads (CLI -d/--min-depth, default 1).
	MinDepth int
	// MinConsensusFraction is the minimum proportion of the most-common
	// base over total usable depth required to call that base; below
	// this, the position is called 'N' (CLI -m/--min-fraction in our
	// CLI; upstream calls this -c/--call-fract).
	MinConsensusFraction float64
	// MinCallFraction is the minimum proportion (score1+score2)/total
	// required to make ANY call (CLI -c/--call-fract). Below this the
	// position is 'N'. Mirrors upstream's `call_fract` (default 0.75).
	MinCallFraction float64
	// MinHetFraction is the minimum proportion of the second-most-common
	// base over the most-common required to call a heterozygous IUPAC
	// ambiguity (CLI -H/--het-fract, default 0.5). Only meaningful with
	// AmbigCodes=true.
	MinHetFraction float64
	// MinMAPQ skips reads with MAPQ below this value (CLI -q/--min-MQ).
	MinMAPQ uint8
	// MinBaseQ skips bases with quality below this value (CLI -Q/--min-BQ).
	MinBaseQ uint8
	...
	LineLen int
	// ShowDel renders deletion placeholder '*' in the consensus sequence.
	// Upstream FASTA/FASTQ default is "no" (do not include the '*');
	// pileup mode always shows them. ShowDel is a regular bool: zero
	// value (false) matches the upstream default.
	ShowDel bool
	// NoShowIns suppresses inserted bases in FASTA/FASTQ output.
	// Upstream's default is "show insertions"; the zero value here
	// (false) reproduces it. Pileup mode never emits insertion-column
	// rows in v1, regardless of this flag.
	NoShowIns bool
	...
	// flags (upstream samtools consensus default; we follow it for
	// consensus and reject the mpileup-style "drop anomalous pair").
	CountOrphans bool
...
// Mirrors MpileupFile's structure: open the input(s), apply the walker,
// and emit per the configured format.
...
// Consensus is the streaming entry point: read records from `in` and emit
// the consensus per the configured format to `out`.
func Consensus(in io.Reader, out io.Writer, opts ConsensusOptions) error {
	...
	// strategy as the mpileup walker (drop unmapped/secondary/etc, apply
	// MAPQ floor), with the consensus-specific tweak that we accept
	// orphan mates by default (matching upstream).
	mpopts := MpileupOptions{
		MinMAPQ:      opts.MinMAPQ,
		MinBaseQ:     opts.MinBaseQ,
		CountOrphans: true, // upstream samtools consensus accepts orphans
	}
	if opts.CountOrphans {
		mpopts.CountOrphans = true
	}
...
// defaults so callers (and tests) can pass a sparse ConsensusOptions.
//
// The defaults model the upstream `samtools consensus -m simple` runtime:
//
//	min-depth=1, min-fraction=0.6, call-fract=0.75, het-fract=0.5,
//	line-len=70, show-ins=yes, show-del=no.
//
// ShowIns is special: a Go bool defaults to false but upstream defaults
// to "yes", so we treat the zero value as "use default" and set true
// unless the CLI has explicitly disabled it. The CLI uses
// `parseYesNo("yes", true)` so it will round-trip the upstream default
// transparently.
func applyConsensusDefaults(opts *ConsensusOptions) {
	...
	if opts.MinConsensusFraction <= 0 {
		opts.MinConsensusFraction = 0.6
	}
	...
	if opts.Format == ConsensusPileup {
		// Pileup mode always shows '*' (matches upstream).
		opts.ShowDel = true
	}
}
...
type consensusCall struct {
	...
	// qual is the per-position confidence as an integer 0..100 (upstream
	// `bam_consensus.c::calculate_consensus_simple` returns
	// `100 * used_score / tscore`). The pileup format emits this number
	// verbatim; FASTA/FASTQ encode it as a Phred+33 byte capped at 93.
	qual  int
...
func emitConsensusContig(bw *bufio.Writer, chrom string, refLen int, windows [][2]int,
	...
	for _, w := range windows {
		...
		// Build per-position event slices for this window. We reuse the
		// mpileup accumulator so the consensus is byte-faithful with
		// what `samtools mpileup` reports.
		width := end0 - beg0
		events := make([][]pileupEvent, width)
		// Lighter copy of MpileupOptions: just what the accumulator
		// reads.
		...
		for pos0 := beg0; pos0 < end0; pos0++ {
			...
			case ConsensusPileup:
				// Pileup mode skips zero-coverage positions unless -a
				// was passed; upstream's basic_pileup also only emits
				// rows when reads are present, with empty_pileup2
				// filling gaps under all_bases.
	...
	if opts.Format != ConsensusPileup {
		...
		// AllPositions on an untouched contig: still emit a record full
		// of N's? Upstream samtools consensus emits a header line and
		// `refLen` N's when -a is set. We follow that.
		if len(seqBuf) == 0 && opts.AllPositions {
...
// callConsensus runs the simple per-position consensus on the event slice.
// Returns a call (base=0 when there's no usable coverage AND the caller
// should treat as a skip) and the usable depth.
func callConsensus(evs []pileupEvent, opts ConsensusOptions) (consensusCall, int) {
	...
	used := call1
	usedScore := s1
	if opts.AmbigCodes && s1 > 0 && float64(s2) >= opts.MinHetFraction*float64(s1) && call2 != 15 {
		used |= call2
		usedScore += s2
	}

	depthOK := totDepth >= opts.MinDepth
	// Two-fraction gate per upstream:
	//   - MinCallFraction guards (usedScore / tscore)
	//   - MinConsensusFraction guards (s1 / tscore) — i.e. the dominant
	//     base must account for at least this fraction of the total.
	majorOK := tscore > 0 && float64(s1) >= opts.MinConsensusFraction*float64(tscore)
	callOK := tscore > 0 && float64(usedScore) >= opts.MinCallFraction*float64(tscore)

	if !depthOK || !majorOK || !callOK {
		if call1 == 16 {
			used = 16
		} else {
			used = 0 // 'N'
	...
	// "N" + IUPAC table (upstream's `het` string).
...
// annotation attached to the current event slice. The simplest faithful
// behaviour (matching upstream's "show_ins yes" / simple mode) is: if a
// majority of reads carry an insertion at this position, emit its
// consensus sequence; otherwise nothing.
func callConsensusInsertion(evs []pileupEvent, opts ConsensusOptions) ([]byte, []byte) {
	// Count reads with vs without an insertion after this position.
	withIns := 0
	insSeqs := [][]byte{}
	live := 0
	for _, e := range evs {
	...
	if float64(withIns)/float64(live) < opts.MinConsensusFraction {
		return nil, nil
	}
	// Per-column majority across all inserted sequences; use the longest
	// observed insertion length as the consensus length, and 'N' for any
	// column with no clear majority.
	maxLen := 0
	...
	for c := 0; c < maxLen; c++ {
		...
		if total == 0 || float64(bestC)/float64(total) < opts.MinConsensusFraction {
			seq[c] = 'N'
			qual[c] = '!'
			continue
		}
		seq[c] = bestB
		// Confidence = (bestC / total)*100 mapped to a Phred+33 byte.
...
// writeConsensusPileupRow emits one row of the samtools consensus pileup
// schema: chrom\tpos\tnth\tdepth\tcall\tcq\tseq\tqual\n. nth is always
// 0 in v1.
func writeConsensusPileupRow(bw *bufio.Writer, chrom string, pos1, depth int,
	...
	// Render the per-read seq / qual strings.
	sortEvents(evs)
	var seq, qual []byte
	for _, e := range evs {
		if e.dropped {
...
// phredByte converts a 0..100 Phred-ish confidence to a printable ASCII
// byte (Phred+33), clamped at 93 so the output stays within printable
// ASCII (33..126).
...
// Pre-computed upstream mapping tables (seqi -> weight on a pure base).
// Mirrors `bam_consensus.c::calculate_consensus_simple::seqi2A/C/G/T`.
//
...
// ParseConsensusMode maps a CLI --mode value to a ConsensusMode.
